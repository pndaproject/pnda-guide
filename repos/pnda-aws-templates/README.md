...
1. Copy ```client_env_example.sh``` and ```pnda_env_example.sh``` to create  ```client_env.sh``` and ```pnda_env.sh```
...
3. Edit pnda_env.sh with the package server IP address. A package server provides a webserver for the binaries for the pnda components, the PNDA guide contains instructions on how to set up a package server.
